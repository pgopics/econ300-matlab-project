...
rng(12, 'twister'); % set random seeds; for more ways to set random seeds, see the documentation of rng

%% load data
% there are many ways to read data; this appears to be the simplest
tsla = readall(datastore('TSLA.csv'));
wmt = readall(datastore('WMT.csv'));
jnj = readall(datastore('JNJ.csv'));

% before calculating the returns, check the data to make sure the time matches

% get the prices
p = [tsla.AdjClose, wmt.AdjClose, jnj.AdjClose];
